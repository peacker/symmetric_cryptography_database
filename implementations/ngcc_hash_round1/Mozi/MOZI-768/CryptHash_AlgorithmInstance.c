...
/// @brief The permutation function of MOZI-768
/// @param[in] state The base address of the internal state
/// @param[in] roundNum The number of rounds to execute (should be equal to 24)
...
/// @brief The hash function based on F-sponge construction
/// @param[in] hash The hash length in BYTES (should be equal to 96)
/// @param[in] rate The rate in BYTES (should be equal to 152)
...
int hash_Fsponge(int hash, int rate, const unsigned char *input, ULL input_len_bits, int output_len_bits, unsigned char *output){
    if (rate != 152 || hash != 96){
...
{
    return hash_Fsponge(digest_len_bits / 8, 152, msg, msg_len_bits, digest_len_bits, digest);
}