...
static void extract_squeeze_block(u64 X[5][5], unsigned char *block) {
    for (int i = 0; i < SQUEEZE_LANE_NUM; i++) {
        int idx = SQUEEZE_LANE_NUM - 1 - i;
        store64_be(block + (i * 8), X[idx % 5][idx / 5]);
    }
}
...
{   
    ...
    while (produced_bits < requested_bits) {
        ...
        unsigned int take_bits = remaining_bits < SQUEEZE_BIT_LENGTH
