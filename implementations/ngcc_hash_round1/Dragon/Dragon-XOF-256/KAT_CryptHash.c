...
static size_t byte_len_from_bit_len(int bit_len);
...
static void fprint_digest(FILE *fp, unsigned char *digest, int digest_len_bits)
{
    ...
    if (NULL != digest)
    {
        int full_bytes = digest_len_bits / 8;
        int remaining_bits = digest_len_bits % 8;
        ...
        if (remaining_bits)
        {
            fprintf(fp, "%02X", digest[full_bytes] & HIGH_N_BIT_MASK(remaining_bits));
        }
    }
    fprintf(fp, "\n");
}
...
static int gen_KAT_2_12(const char *algorithm_instance_name, const int digest_len_bits)
{
    ...
    digest = (unsigned char *)malloc(byte_len_from_bit_len(digest_len_bits));
    if (NULL == digest)
    {
        fprintf(stderr, "\nERROR: Memory allocation failed at %s, line %d. \n", __FILE__, __LINE__);
        rt = KAT_MEMORY_ALLOCATION_FAILED;
    ...
    memset(digest, 0, byte_len_from_bit_len(digest_len_bits));
...
static int gen_KAT_Loop(const char *algorithm_instance_name, const int digest_len_bits)
{
    ...
    buffer = (unsigned char *)malloc(byte_len_from_bit_len(digest_len_bits));
    if (NULL == buffer)
    {
        fprintf(stderr, "\nERROR: Memory allocation failed at %s, line %d. \n", __FILE__, __LINE__);
        rt = KAT_MEMORY_ALLOCATION_FAILED;
