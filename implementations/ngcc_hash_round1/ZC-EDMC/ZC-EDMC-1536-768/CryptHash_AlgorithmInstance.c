...
#include "../../../lib/low/ZuD-1536/plain/ZuD1536-plain.h"

/* Width of state in bits */
#define WIDTH_BITS 1536u
#define WIDTH_BYTES (WIDTH_BITS/8u)

static void permute(ZuD1536_plain64_state *s)
{
    ZuD1536_plain_Permute_12rounds(s);
}
static void permute_half(ZuD1536_plain64_state *s)
{
    ZuD1536_plain_Permute_6rounds(s);
}
...
/* Forward declaration: used by append_byte_msb_custom. */
static void absorb_block_custom(ZuD1536_plain64_state *state, const uint8_t *block, unsigned int rateBytes);

static void append_bit_msb_custom(
    ZuD1536_plain64_state *st,
    uint8_t *block,
    unsigned int *bitpos,
...
static void absorb_block_custom(ZuD1536_plain64_state *state, const uint8_t *block, unsigned int rateBytes)
{
    ZuD1536_plain_AddBytes(state, block, 0, rateBytes);
    ...
    ZuD1536_plain_AddBytes(state, rateBytes + oldState, rateBytes, WIDTH_BYTES - rateBytes);

    permute_half(state);
}

/* Squeeze digest_len_bits (must be byte-aligned for this harness). */
static int squeeze_bytes(ZuD1536_plain64_state *state, unsigned int rateBytes, uint8_t *out, unsigned int outBytes)
{
    unsigned int produced = 0;
    while (produced < outBytes) {
        unsigned int take = outBytes - produced;
        if (take > rateBytes) take = rateBytes;
        ZuD1536_plain_ExtractBytes(state, out + produced, 0, take);
        produced += take;
...
int CryptHash(int digest_len_bits, const unsigned char *msg, unsigned long long msg_len_bits, unsigned char *digest)
{
    ...
    /* SHA3 delimited suffix, treated as 8 bits appended after the message bits. */
    //const uint8_t suffix = 0x06;

    ZuD1536_plain64_state st;
    ZuD1536_plain_Initialize(&st);
    ...
    uint8_t block[/* VLA not allowed in some toolchains; use max */ 192];
