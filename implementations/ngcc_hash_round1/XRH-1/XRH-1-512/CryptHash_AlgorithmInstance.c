...
void XRH_edmc_permute(uint64_t state[STATE_LANES]) {
    volatile uint64_t initial_state[STATE_LANES];
    memcpy((void*)initial_state, state, sizeof(uint64_t) * STATE_LANES);
    XRH1280(0, 9, state);
    for (int j = STATE_LANES - (STATE_CAPACITY / 64); j < STATE_LANES; j++) state[j] ^= initial_state[j];
    XRH1280(9, 18, state);
}

static void XRH_edmc_init(XRH_state *state, uint32_t rate_bits) {
...
int CryptHash(int digest_len_bits, const unsigned char *msg, unsigned long long msg_len_bits, unsigned char *digest)
{
    ...
    XRH_edmc_init(&state, STATE_RATE);
