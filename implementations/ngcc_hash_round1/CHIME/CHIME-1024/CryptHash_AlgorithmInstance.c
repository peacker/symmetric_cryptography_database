...
/*
 * Computes a 1024-bit CHIME digest for a bit-length message.
 * Inputs:
 *     msg: message bytes
 *     msg_len_bits: message length in bit
 * Outputs:
 *     digest: 1024-bit digest buffer
 *     return value: 0 on success, -1 on failure
 *
 * 计算按位长度消息的 1024 位 CHIME 摘要。
 * 输入：
 *     msg：消息字节流
 *     msg_len_bits：消息位长度
 * 输出：
 ...
 *     返回值：成功返回 0，失败返回 -1
 */
int CryptHash1024(const unsigned char *msg, unsigned long long msg_len_bits, unsigned char *digest)
{
    return chime_1024_hash_bits((const uint8_t*)msg, msg_len_bits, (uint8_t*)digest);
}

// ==================== Permutation without AVX or SSE ====================
 ...
 *
 * 对三个 4-lane 向量执行 chi 非线性运算并原地更新。
 * 输入：
 *     A：更新前的第一组状态向量
 *     B：更新前的第二组状态向量
 *     C：更新前的第三组状态向量
...
/* ============================================================
 * Padded block builder
 * Each block occupies 64 bytes, but only low 56 bytes are used.
 * ============================================================ */
...
/*
 * Builds padded CHIME-1024 blocks with bit-level pad10*1 rule.
 * Inputs:
 ...
 *
 * 按位级 pad10*1 规则构建 CHIME-1024 填充块。
 * 输入：
 *     msg：消息比特流
 *     len_bits：消息位长度
 * 输出：
 ...
 */
int chime_1024_build_padded_blocks_bit_level(
    const uint8_t* msg,
    ...
    *num_blocks_out = 0;
    ...
    full_blocks_ull = len_bits / CHIME_1024_RATE_BITS;
    rem_bits_ull    = len_bits % CHIME_1024_RATE_BITS;

    if (rem_bits_ull <= CHIME_1024_RATE_BITS - 2u) {
        num_blocks_ull = full_blocks_ull + 1u;
    } else if (rem_bits_ull == CHIME_1024_RATE_BITS - 1u) {
    ...
    if (num_blocks_sz > SIZE_MAX / CHIME_1024_BLOCK_BYTES) return -1;

    blocks = (uint8_t*)calloc(num_blocks_sz, CHIME_1024_BLOCK_BYTES);
    if (blocks == NULL) return -1;

    /* Copy all full 56-byte blocks directly */
    for (size_t i = 0; i < (size_t)full_blocks_ull; i++) {
        memcpy(blocks + i * CHIME_1024_BLOCK_BYTES,
               msg + i * CHIME_1024_RATE_BYTES,
               CHIME_1024_RATE_BYTES);
    }

    if (rem_bits_ull <= CHIME_1024_RATE_BITS - 2u) {
        uint8_t* blk = blocks + (size_t)full_blocks_ull * CHIME_1024_BLOCK_BYTES;
        unsigned long long full_bytes = rem_bits_ull >> 3;
        unsigned rem_bits = (unsigned)(rem_bits_ull & 7u);

        copy_bits_msb_prefix(blk, msg + (size_t)full_blocks_ull * CHIME_1024_RATE_BYTES, rem_bits_ull);
        ...
        /* final '1' = bit 447 = byte 55 bit 0 */
        blk[CHIME_1024_RATE_BYTES - 1] |= 0x01u;
    }
    else if (rem_bits_ull == CHIME_1024_RATE_BITS - 1u) {
        uint8_t* blk0 = blocks + (size_t)full_blocks_ull * CHIME_1024_BLOCK_BYTES;
        uint8_t* blk1 = blocks + ((size_t)full_blocks_ull + 1u) * CHIME_1024_BLOCK_BYTES;

        copy_bits_msb_prefix(blk0, msg + (size_t)full_blocks_ull * CHIME_1024_RATE_BYTES, rem_bits_ull);

        blk0[CHIME_1024_RATE_BYTES - 1] |= 0x01u;
        blk1[CHIME_1024_RATE_BYTES - 1] |= 0x01u;
    }
    else {
        uint8_t* blk = blocks + (size_t)full_blocks_ull * CHIME_1024_BLOCK_BYTES;
        blk[0] |= 0x80u;
 ...
 *     blocks：待释放的堆缓冲区
 */
void chime_1024_free_blocks(uint8_t* blocks) {
    free(blocks);
}

/*
 * Loads one 448-bit rate block into A0 and A1 lanes under the CHIME-1024 mapping.
 * Inputs:
 *     block64: 64-byte block buffer (low 56 bytes meaningful)
 * Outputs:
 *     A0: first 256-bit lane group from block[0..31]
 *     A1: lane group where A1[0] is zero and A1[1..3] come from block[32..55]
 *
 * 按 CHIME-1024 映射将一个 448 位速率块装载到 A0 和 A1 lane。
 * 输入：
 *     block64：64 字节块缓冲区（低 56 字节有效）
 * 输出：
 *     A0：来自 block[0..31] 的首个 256 位 lane 组
 *     A1：A1[0] 置零，A1[1..3] 来自 block[32..55] 的 lane 组
 */
static void load_rate448_to_A0_A1(
    ...
    /* A1[63:0] = 0, A1[255:64] = next 24 bytes */
    A1[0] = 0u;
    A1[1] = load64_le(block64 + 32);
    A1[2] = load64_le(block64 + 40);
    A1[3] = load64_le(block64 + 48);
}

/*
 * Applies feed-forward on the 1088-bit capacity portion only.
 * Inputs:
 *     old_s: state snapshot before permutation
 *     new_s: state after permutation before feed-forward
 * Outputs:
 *     new_s: updated in place with capacity-only feed-forward
 *
 * 仅对 1088 位容量部分执行前馈。
 * 输入：
 *     old_s：置换前的状态快照
 ...
 *     new_s：容量部分前馈后原地更新
 */
static void feedforward_inner_1088(const p_state_t* old_s, p_state_t* new_s) {
    new_s->A1[0] ^= old_s->A1[0];
    xor64x4(new_s->B0, old_s->B0);
    xor64x4(new_s->B1, old_s->B1);
    xor64x4(new_s->C0, old_s->C0);
    xor64x4(new_s->C1, old_s->C1);
}

/*
 * Absorbs all input blocks into the state and applies 20 rounds per block.
 * Inputs:
 ...
 */
void chime_1024_p_448_1088_blocks_ref(
    ...
    for (i = 0; i < num_blocks; i++) {
        const uint8_t* blk = blocks + i * CHIME_1024_BLOCK_BYTES;

        if (i == num_blocks - 1) {
            // xor 0^{c-1}||1 to the capacity part to prevent length extension attacks
            s.C1[0] ^= 1ull;
        }
        p_state_t old_s = s;
...
/*
 * Extracts a 1024-bit digest from state lanes B0, B1, C0 and C1.
 * Inputs:
 *     s: state to read digest from
 * Outputs:
 *     digest: 128-byte output digest
 *
 * 从状态的 B0、B1、C0 和 C1 lane 中提取 1024 位摘要。
 * 输入：
 *     s：用于读取摘要的状态
 * 输出：
 *     digest：128 字节输出摘要
 */
void chime_1024_extract_digest1024(
    const p_state_t* s,
    uint8_t digest[CHIME_1024_DIGEST_BYTES]
) {
    /* low 256 bits = B0 */
    store64_le(digest +   0, s->B0[0]);
    store64_le(digest +   8, s->B0[1]);
    store64_le(digest +  16, s->B0[2]);
    store64_le(digest +  24, s->B0[3]);

    /* next 256 bits = B1 */
    store64_le(digest +  32, s->B1[0]);
    store64_le(digest +  40, s->B1[1]);
    store64_le(digest +  48, s->B1[2]);
    store64_le(digest +  56, s->B1[3]);

    /* next 256 bits = C0 */
    store64_le(digest +  64, s->C0[0]);
    store64_le(digest +  72, s->C0[1]);
    store64_le(digest +  80, s->C0[2]);
    store64_le(digest +  88, s->C0[3]);

    /* high 256 bits = C1 */
    store64_le(digest +  96, s->C1[0]);
    store64_le(digest + 104, s->C1[1]);
    store64_le(digest + 112, s->C1[2]);
    store64_le(digest + 120, s->C1[3]);
}

/*
 * Performs complete CHIME-1024 hashing: padding, absorption, and digest extraction.
 * Inputs:
 *     msg: message bitstream
 *     len_bits: message bit length
 * Outputs:
 *     digest: final 1024-bit digest
 *     return value: 0 on success, -1 on failure
 *
 * 执行完整 CHIME-1024 哈希流程：填充、吸收、摘要提取。
 * 输入：
 *     msg：消息比特流
 *     len_bits：消息位长度
 * 输出：
...
int chime_1024_hash_bits(
    ...
    rc = chime_1024_build_padded_blocks_bit_level(msg, len_bits, &blocks, &num_blocks);
    if (rc != 0) return -1;

    chime_1024_p_448_1088_blocks_ref(blocks, num_blocks, &s);
    chime_1024_extract_digest1024(&s, digest);

    chime_1024_free_blocks(blocks);
    return 0;
}

// =========================== END: CHIME-1024 implementation ===========================
