...
- **Instances**: one per named concrete parameterization the source actually specifies (not one per every number mentioned) — with `characteristics` sized/rounded correctly, and per-instance `reference_ids` when a specific instance is only defined in one of several cited documents (e.g. a NIST-standardized profile with a later doc than the original design).
  - **`<field>_min` and `<field>_max` are all-or-nothing.** `scripts/validate.py:validate_range_field` rejects an instance that defines one without the other, for *every* range field (`key_size_bits`, `block_size_bits`, `state_size_bits`, `rounds`, `output_size_bits`, `tweak*`). So a design the source describes only as "up to N bits" cannot be recorded as a bare `key_size_bits_max: N` — either the source also gives the floor (then use the pair), or record the single representative/maximum value in the plain `key_size_bits` field and say in the instance's `notes` that it is a maximum rather than a fixed size. Inventing a floor the source never states, just to satisfy the pair, is worse than either.
  - Some fields the schema permits are also legitimately *omissible*: ~80 existing instances carry no `rounds` at all. When a source deliberately declines to recommend a round count (analysis incomplete, parameter left to the user), omit the field and explain why in `notes` rather than inventing a number or promoting an illustrative example from the text into a recommendation.
- **A mechanical YAML trap worth pre-empting**: `notes`/`note` fields in this database are quotation-heavy, and a plain (unquoted) YAML scalar containing `": "` — which is exactly what "the paper states outright: "..."" produces — parses as a nested mapping and fails with "Nested mappings are not allowed in compact mappings". Any note that introduces a quotation with a colon, or contains a `#` mid-line, must be single-quoted as a whole with internal apostrophes doubled (`''`), matching the style already used throughout `data/*.yaml`. Re-running `validate.py` after each entry (per the intro to this section) catches it immediately; writing three entries first and then parsing does not tell you which one broke.
