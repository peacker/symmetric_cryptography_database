...
| Root | Definition |
|---|---|
| Merkle–Damgård Construction (`merkle_damgard`) → HAIFA, MDP, **Array-Folding Iterated Hash** | Iterated hashing: a fixed-input compression function applied to successive message blocks while carrying a chaining value, with length-encoding strengthening padding. The HAIFA leaf (`haifa`, Biham and Dunkelman 2006) additionally mixes a running bit counter (and optional salt) into every compression call, closing the generic multicollision/long-message second-preimage attacks that apply to plain MD. The MDP leaf (`mdp`, Hirose, Park, Yun, ASIACRYPT 2007) instead applies a public permutation to the chaining value entering only the *final* compression call, blocking length-extension for the cost of one extra permutation rather than a doubled compression function (MoFang). The Array-Folding leaf (`array_folding_iterated_hash`, Snefru) replaces the single chaining variable with a small fixed-size array of chaining slots: message blocks are compressed independently into successive slots, and the array is folded back into slot 0 by the same function whenever it fills. |
| Sponge Construction (`sponge_construction`) → Standard Sponge, **Feed-Forward Sponge** | Absorbs input into part of a fixed-size state (the rate, alongside an untouched capacity) interleaved with permutation calls, then squeezes output the same way; unkeyed/unidirectional by default. The Feed-Forward Sponge leaf (`feedforward_sponge`) XORs the pre-call capacity back into the post-call capacity at every absorbing step, turning each call into a Davies-Meyer-style one-way map restricted to the capacity — tightening (second-)preimage resistance without enlarging the state. Several 2025–2026 papers describe this independently under different names (Sponge-F, Sponge-DM, Sponge-EDM); families citing a specific one record the citation via `reference_ids`, not an `influences` edge, since the paper is a mode construction rather than a family in this database. |
...
| Interleaved Feistel Sponge (`interleaved_feistel_sponge`) | A rate branch plus two capacity branches, where each "Step" makes two permutation calls (typically domain-separated instances of the same permutation) whose capacity outputs are coupled Feistel-style rather than fed straight through — neither a plain sponge (one call/step) nor a classical Feistel network (no permutation calls at all) (TaiChi). |
...
| Native Register Hash (`native_register_hash`) | The hash-mode counterpart of Native Keystream Generator below: message bits are injected directly into the feedback taps of a register network (typically NFSR-based) each "beat," with no separable fixed-size permutation/compression function, and the digest is read out over a further constant-driven finalization phase over the same network (AXIS). |
| Block-Cipher-Based Hash Construction (`block_cipher_based_hash`) → **Counter-bDM** (`counter_bdm`) | Generalizes the PGV/Davies-Meyer feedforward pattern to hashing via an independently-defined block-cipher primitive (unlike the primitive-tier `davies_meyer`/PGV leaves, which classify a single-call compression function as the primitive itself when no separate block-cipher family exists). Counter-bDM (Abed, Forler, List, Lucks, Wenzel, AFRICACRYPT 2014) runs b parallel block-cipher calls per iteration with the message driving the key and counter-differentiated per-branch plaintexts for domain separation (uHash, b∈{2,3,4} over uBlock-1024). |
...
| Number-Theoretic Hash Construction (`number_theoretic_hash_construction`) → Discrete-Log-Hardness Hash, Lattice-Hardness Hash, **Group-Theoretic (Cayley Hash) Hardness Hash** | For designs whose collision resistance reduces to a number-theoretic hardness assumption and which have no separate fixed-size primitive worth modeling of its own — e.g. MuHASH/AdHASH/LtHASH's "randomizing" step is just any ideal generic hash, and the group-theoretic Group-Theoretic leaf (`group_theoretic_hardness_hash`, CHAMP) drives a left-to-right product of two fixed generator matrices in a group directly by the message bits, with no fixed-size primitive at all. |
| Randomize-then-Combine Construction (`randomize_then_combine`) | Bellare and Micciancio's own name: each message block x_i is passed through a fixed, ideal randomizing function h to give y_i = h((i).x_i), and the y_i are combined via an associative group operation — parallel and natively incremental, unlike Merkle-Damgård (MuHASH, AdHASH, LtHASH). |
