...
| Root | Definition |
|---|---|
| Merkle–Damgård Construction (`merkle_damgard`) → HAIFA, **Array-Folding Iterated Hash** | Iterated hashing: a fixed-input compression function applied to successive message blocks while carrying a chaining value, with length-encoding strengthening padding. The HAIFA leaf (`haifa`, Biham and Dunkelman 2006) additionally mixes a running bit counter (and optional salt) into every compression call, closing the generic multicollision/long-message second-preimage attacks that apply to plain MD. The Array-Folding leaf (`array_folding_iterated_hash`, Snefru) replaces the single chaining variable with a small fixed-size array of chaining slots: message blocks are compressed independently into successive slots, and the array is folded back into slot 0 by the same function whenever it fills. |
