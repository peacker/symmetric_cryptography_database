...
| Root | Definition |
|---|---|
| Merkle–Damgård Construction (`merkle_damgard`) → **Array-Folding Iterated Hash** | Iterated hashing: a fixed-input compression function applied to successive message blocks while carrying a chaining value, with length-encoding strengthening padding. The Array-Folding leaf (`array_folding_iterated_hash`, Snefru) replaces the single chaining variable with a small fixed-size array of chaining slots: message blocks are compressed independently into successive slots, and the array is folded back into slot 0 by the same function whenever it fills. |
